...
class PlanningAgent:
    ...
    def _semantic_route(self, query: str) -> Optional[str]:
        tool_name = self.vector_db.search(query, similarity_threshold=0)
    ...
    async def run(self, user_input: str):
        ...
        # STEP 2: CHAT
        if not self.active_tool_name:
            messages = [SystemMessage(content=self.base_prompt)] + self.history + [HumanMessage(content=user_input)]
        ...
        param_details = "\n".join([f"- {p.name} ({'Required' if p.required else 'Optional'}): {p.description}" for p in tool_obj.parameters])
        ...
        system_prompt = f"""
            ...
            2. Map user's natural language to the specific parameter values allowed.
               - Synonyms mappings: 'make'/'generate' -> 'create'; 'insert' -> 'add' (or 'create'); 'drop' -> 'delete'; 'give'/'fetch' -> 'get'; 'change' -> 'update'.
            3. If a parameter is missing and cannot be inferred, ask user strictly for missing parameters.
            4. If ALL present, return JSON:
            ...
            try:
                ...
                if inspect.isasyncgenfunction(func):
                    async for item in func(**tool_call["arguments"]):
                        print(item) # Output to console immediately
                        yield str(item)
