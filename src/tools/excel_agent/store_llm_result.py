    ...
    def func(unique_id):
        ...
        async def store_llm_result(
            ...
            try:
                ...
                try:
                    conn.execute(f'DROP TABLE IF EXISTS "{lookup_table}";')
                except Exception:
                    pass
                ...
                if safe_result_col not in col_names:
                    try:
                        conn.execute(f'ALTER TABLE "{main_table}" ADD COLUMN "{safe_result_col}" {result_type};')
                        conn.commit()
        ...
        return DynamicTool(
            ...
            parameters=[
                ...
                ToolParam(
                    name="result_type",
                    type="string",
                    description="SQLite type: TEXT, INTEGER, or REAL (default: TEXT)",
                    required=False,
                ),
            ],
